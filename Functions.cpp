...
void CalculatePlayerVertex(Player& player) {
	player.vertex.LeftTop.x = player.vertex.center.x - player.vertex.width / 2;
	player.vertex.LeftTop.y = player.vertex.center.y - player.vertex.height / 2;
	player.vertex.LeftBottom.x = player.vertex.center.x - player.vertex.width / 2;
	player.vertex.LeftBottom.y = player.vertex.center.y + player.vertex.height / 2;
	player.vertex.RightTop.x = player.vertex.center.x + player.vertex.width / 2;
	player.vertex.RightTop.y = player.vertex.center.y - player.vertex.height / 2;
	player.vertex.RightBottom.x = player.vertex.center.x + player.vertex.width / 2;
	player.vertex.RightBottom.y = player.vertex.center.y + player.vertex.height / 2;
}

void newDrawQuad(Vertex vertex, int textureHandle) {
	Novice::DrawQuad(int(vertex.LeftTop.x), int(vertex.LeftTop.y), int(vertex.RightTop.x), int(vertex.RightTop.y), int(vertex.LeftBottom.x), int(vertex.LeftBottom.y), int(vertex.RightBottom.x), int(vertex.RightBottom.y), 0, 0, int(vertex.width), int(vertex.height), textureHandle, 0xFFFFFFFF);
}

void CalculateQuadMapMapNumber(Player& player, const int MapChipSize) {
	player.vertex.mapnum.LeftTop.x = int(player.vertex.LeftTop.x / MapChipSize);
	player.vertex.mapnum.LeftTop.y = int(player.vertex.LeftTop.y / MapChipSize);
	player.vertex.mapnum.LeftBottom.x = int(player.vertex.LeftBottom.x / MapChipSize);
	player.vertex.mapnum.LeftBottom.y = int(player.vertex.LeftBottom.y / MapChipSize);
	player.vertex.mapnum.RightTop.x = int(player.vertex.RightTop.x / MapChipSize);
	player.vertex.mapnum.RightTop.y = int(player.vertex.RightTop.y / MapChipSize);
	player.vertex.mapnum.RightBottom.x = int(player.vertex.RightBottom.x / MapChipSize);
	player.vertex.mapnum.RightBottom.y = int(player.vertex.RightBottom.y / MapChipSize);
}

void VertexScreenPrintf(Player player, int x, int y) {
	int xTmp = 60;
	int yTmp = 25;
	Novice::ScreenPrintf(x, y, "%d", int(player.vertex.mapnum.LeftTop.x));
	Novice::ScreenPrintf(x + xTmp, y, "%d", int(player.vertex.mapnum.LeftTop.y));
	Novice::ScreenPrintf(x, y + yTmp, "%d", int(player.vertex.mapnum.LeftBottom.x));
	Novice::ScreenPrintf(x + xTmp, y + yTmp, "%d", int(player.vertex.mapnum.LeftBottom.y));
	Novice::ScreenPrintf(x, y + yTmp * 2, "%d", int(player.vertex.mapnum.RightTop.x));
	Novice::ScreenPrintf(x + xTmp, y + yTmp * 2, "%d", int(player.vertex.mapnum.RightTop.y));
	Novice::ScreenPrintf(x, y + yTmp * 3, "%d", int(player.vertex.mapnum.RightBottom.x));
	Novice::ScreenPrintf(x + xTmp, y + yTmp * 3, "%d", int(player.vertex.mapnum.RightBottom.y));
}

//void MapChipCollision(Player& player, const int MapChipSize, const int MapSizeWidth, const int MapSizeHeight,int mapchip[20][10]) {
//	if (mapchip[player.vertex.mapnum.RightTop.x][player.vertex.mapnum.RightBottom.x] == 0) {
//		player.vertex.center
//	}
//}